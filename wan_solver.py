...
class wan_pde_solver(nn.Module):
    def __init__(self, N_dm, N_bd, beta_int, beta_bd,
                 v_step, v_lr, u_step, u_lr, dim=2, x_range=[-1,1], t_range=[0,1], u_layer=6, u_neuron=40,
                 iteration=20000):
        super().__init__()
    ...
    def sample_train(self, dm_size, bd_size, dim=2):
        ...
        x_dm = torch.cat((x_dm, t_dm), dim=1)
        # value of u, f , dim: [x_inside_size,x_inside_size, t_size]
        u_true = 2*torch.sin(torch.pi/2.*x_dm[:,0])*torch.cos(torch.pi/2.*x_dm[:,1])*torch.exp(-x_dm[:,-1])
        f_obv = (torch.pi**2/2.-1.) * u_true

        ## initial condition h(x)
        x_init = torch.distributions.uniform.Uniform(self.x_l,self.x_r).sample([bd_size,dim])
        x_init = torch.cat((x_init, torch.ones(bd_size, 1)* self.t0),dim=1)
        x_end = torch.distributions.uniform.Uniform(self.x_l,self.x_r).sample([bd_size,dim])
        x_end = torch.cat((x_end, torch.ones(bd_size, 1)* self.t1),dim=1)
        ...
        return (x_dm, x_init, x_end, x_bd, f_obv.view(-1,1), h_obv.view(-1,1), bd_obv.view(-1,1))
    ...
    def constraints_w(self, x_in, diff=False):
        # decay function w = e^(-1/I/(x^2-1))  Boundary tends to 0
        # so test function with constraints: p = w*v
        I1 = 0.210987 #decay_coefficient
        w = torch.exp(-1./(I1*(x_in[:,0]**2-1)*(x_in[:,1]**2-1)))
        if diff:
            # dwx = 2x/(I(x**2-1)**2) * w
            dw_x = 2.* x_in[:,0] / (I1 * (x_in[:,0]**2 -1 )**2 * (x_in[:,1]**2-1)) * w
            dw_y = 2.* x_in[:,1] / (I1 * (x_in[:,1]**2 -1 )**2 * (x_in[:,0]**2-1)) * w
    ...
    def build(self, x_dm, x_init, x_end, x_bd, f_obv, h_obv, bd_obv):
        ...
        test_norm = torch.mean(torch.square(p_dm)) * int_dm
        ## integral term: 
        ## 
    ...
    def main_fun(self):
        ...
        for itr in range(self.iteration):
            ...
            for _ in range(self.v_step):
                loss_u, loss_v, loss_int, loss_bd = self.build(x_dm, x_init, x_end, x_bd, f_obv, h_obv, bd_obv)
            ...
            if itr % 5==0:
                ...
                print(f'loss_u:{loss_u} loss_v:{loss_v} loss_int:{loss_int} loss_bd:{loss_bd} test_norm:{err_l2}')
